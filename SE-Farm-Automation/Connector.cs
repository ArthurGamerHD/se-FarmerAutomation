...
namespace FarmerAutomation
{
    ...
    {
        const string ID_PREFIX = nameof(FarmerAutomation) + "_";
        ...
        {
            var button =
                MyAPIGateway.TerminalControls.CreateControl<IMyTerminalControlButton, IMyShipConnector>(ID_PREFIX +
                    "DropOnceButton");
        ...
        {
            {
                ...
                a.Writer = (b, sb) =>
                {
                    ...
                    var status =
                        connector == null
                            ? MyTexts.GetString("DetectedEntity_Unknown") // Connector status is Unknown ("Unknown")
                            : !connector.Enabled
                                ? MyTexts.GetString("SwitchText_Off") // Disabled ("Off")
                                : b.GetInventory(0).Empty()
                                    ? MyTexts.GetString("BlockPropertyProperties_WaterLevel_Empty") // Empty ("Empty")
                                    : (connector.Status != MyShipConnectorStatus.Unconnected)
                                        ? MyTexts.GetString("EventState_CockpitOccupied") // Is Connected ("Occupied")
                                        : MyTexts.GetString("ScreenMedicals_RespawnShipReady"); // Is Ready ("Ready")
        ...
        {
            if (connector.Status != MyShipConnectorStatus.Unconnected || !connector.Enabled ||
                !connector.IsFunctional || connector.Closed)
                return;
            ...
            Vector3D spawnPos = connector.GetPosition() +
                                wm.Forward * (connector.CubeGrid.GridSize * 0.5 + halfSize + 0.1);
            Vector3D forward = wm.Forward;
            Vector3D up = wm.Up;

            MyFloatingObjects.Spawn(droppedItem, spawnPos, forward, up, connector.CubeGrid?.Physics,
                entity => { entity.Physics.LinearVelocity += (Vector3)(wm.Forward * 2f); });
        }
    }
}